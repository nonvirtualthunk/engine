 ...
 * operations.  {@link NonBlockingHashMapi} scales substatially better than
...
public class NonBlockingHashMapi<TypeK, TypeV>
  ...
  static {                      // <clinit>
    Field f = null;
    try { f = NonBlockingHashMapi.class.getDeclaredField("_kvs"); }
  ...
  // Helper function to spread lousy hashCodes
  private static final int hashi(final int key) {
    int h = key;     // The real hashCode call
    // Spread bits to regularize both segment and index locations,
    // using variant of single-word Wang/Jenkins hash.
    h += (h <<  15) ^ 0xffffcd7d;
    h ^= (h >>> 10);
  ...
  /** Create a new NonBlockingHashMapi with default minimum size (currently set
   *  to 8 K/V pairs or roughly 84 bytes on a standard 32-bit JVM). */
  public NonBlockingHashMapi( ) { this(MIN_SIZE); }

  /** Create a new NonBlockingHashMapi with initial room for the given number of
  ...
  public NonBlockingHashMapi( final int initial_sz ) { initialize(initial_sz); }
  ...
  public TypeV   putIfAbsent( TypeK  key, TypeV val ) { return putIfMatch( key,      val, TOMBSTONE   ); }
    ...
    public TypeV   putIfAbsenti( int  key, TypeV val ) { return putIfMatchi( key,      val, TOMBSTONE   ); }
  ...
  public boolean replace    ( TypeK  key, TypeV  oldValue, TypeV newValue ) {
     ...
     *  mapped a value which is <code>equals</code> to <code>oldValue</code>.
     *  @throws NullPointerException if the specified key or value is null */
    public boolean replacei    ( int key, TypeV  oldValue, TypeV newValue ) {
  	  final Object out = putIfMatchi(key, newValue, oldValue);
  	  return (oldValue == null) ? out == oldValue : oldValue.equals(out);
    }
  ...
  private final TypeV putIfMatch( Object key, Object newVal, Object oldVal ) {
    ...
    private final TypeV putIfMatchi( int key, Object newVal, Object oldVal ) {
    if (oldVal == null || newVal == null) throw new NullPointerException();
    final Object res = putIfMatchi( this, _kvs, key, newVal, oldVal );
    assert !(res instanceof Prime);
    assert res != null;
    return res == TOMBSTONE ? null : (TypeV)res;
  }
  ...
  public void clear() {         // Smack a new empty table down
    Object[] newkvs = new NonBlockingHashMapi(MIN_SIZE)._kvs;
  ...
  public Object clone() {
    try {
      ...
      NonBlockingHashMapi<TypeK,TypeV> t = (NonBlockingHashMapi<TypeK,TypeV>) super.clone();
  ...
  // the hashes are unequal (fast negative test) and finally do the full-on
  // 'equals' v-call.
  private static boolean keyeqi( Object K, int key, int[] hashes, int hash, int fullhash ) {
    return K!=null && K != TOMBSTONE && ((Integer)K).intValue() == key;
  }
  ...
  public TypeV get( Object key ) {
    ...
    public TypeV geti( int key ) {
      final int fullhash = hashi (key); // throws NullPointerException if key is null
      final Object V = get_impli(this,_kvs,key,fullhash);
      assert !(V instanceof Prime); // Never return a Prime
      return (TypeV)V;
    }


  private static final Object get_impl( final NonBlockingHashMapi topmap, final Object[] kvs, final Object key, final int fullhash ) {
  ...
  private static final Object get_impli( final NonBlockingHashMapi topmap, final Object[] kvs, final int key, final int fullhash ) {
    final int len     = len  (kvs); // Count of key/value pairs, reads kvs.length
    final CHM chm     = chm  (kvs); // The CHM, for a volatile read below; reads slot 0 of kvs
    final int[] hashes=hashes(kvs); // The memoized hashes; reads slot 1 of kvs
    ...
    while( true ) {
      ...
      // Key-compare
      if( keyeqi(K,key,hashes,idx,fullhash) ) {
        // Key hit!  Check for no table-copy-in-progress
        if( !(V instanceof Prime) ) // No copy?
          return (V == TOMBSTONE) ? null : V; // Return the value
        // Key hit - but slot is (possibly partially) copied to the new table.
        // Finish the copy & retry in the new table.
        return get_impli(topmap,chm.copy_slot_and_check(topmap,kvs,idx,key),key,fullhash); // Retry in the new table
      }
      // get and put must have the same key lookup logic!  But only 'put'
      // needs to force a table-resize for a too-long key-reprobe sequence.
      // Check for too-many-reprobes on get - and flip to the new table.
      if( ++reprobe_cnt >= reprobe_limit(len)// too many probes
        ...
        return newkvs == null ? null : get_impli(topmap,topmap.help_copy(newkvs),key,fullhash); // Retry in the new table
  ...
  private static final Object putIfMatch( final NonBlockingHashMapi topmap, final Object[] kvs, final Object key, final Object putval, final Object expVal ) {
    assert putval != null;
    ...
    private static final Object putIfMatchi( final NonBlockingHashMapi topmap, final Object[] kvs, final int key, final Object putval, final Object expVal ) {
    assert putval != null;
    assert !(putval instanceof Prime);
    assert !(expVal instanceof Prime);
    final int fullhash = hashi(key); // throws NullPointerException if key null
    final int len      = len   (kvs); // Count of key/value pairs, reads kvs.length
    final CHM chm      = chm   (kvs); // Reads kvs[0]
    final int[] hashes = hashes(kvs); // Reads kvs[1], read before kvs[0]
    int idx = fullhash & (len-1);
    ...
    while( true ) {             // Spin till we get a Key slot
      ...
      //topmap._reprobes.add(1);
      if( ++reprobe_cnt >= reprobe_limit(len) // too many probes or
          /*key == TOMBSTONE removed tombstone check, this would make this unsafe, if we removed keys, which we don't at present*/ ) { // found a TOMBSTONE key, means no more keys
        // We simply must have a new table to do a 'put'.  At this point a
        // 'get' will also go to the new table (if any).  We do not need
        // to claim a key slot (indeed, we cannot find a free one to claim!).
        newkvs = chm.resize(topmap,kvs);
        if( expVal != null ) topmap.help_copy(newkvs); // help along an existing copy
        return putIfMatchi(topmap, newkvs, key, putval, expVal);
      }
    ...
    // If so, copy our slot and retry in the new table.
    if( newkvs != null )
      return putIfMatchi(topmap, chm.copy_slot_and_check(topmap, kvs, idx, expVal), key, putval, expVal);
  ...
  // --- CHM -----------------------------------------------------------------
  // The control structure for the NonBlockingHashMapi
  private static final class CHM<TypeK,TypeV> {
    ...
    private final Object[] resize( NonBlockingHashMapi topmap, Object[] kvs) {
      assert chm(kvs) == this;
    ...
    // of the top position.
    private final void help_copy_impl( NonBlockingHashMapi topmap, Object[] oldkvs, boolean copy_all ) {
    ...
    // read it yet.
    private final Object[] copy_slot_and_check( NonBlockingHashMapi topmap, Object[] oldkvs, int idx, Object should_help ) {
      assert chm(oldkvs) == this;
      Object[] newkvs = _newkvs; // VOLATILE READ
    ...
    private final void copy_check_and_promote( NonBlockingHashMapi topmap, Object[] oldkvs, int workdone ) {
      assert chm(oldkvs) == this;
    ...
    private boolean copy_slot( NonBlockingHashMapi topmap, int idx, Object[] oldkvs, Object[] newkvs ) {
  ...
  private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
    ...
    public SnapshotV() {
      while( true ) {           // Verify no table-copy-in-progress
        ...
        topchm.help_copy_impl(NonBlockingHashMapi.this,topkvs,true);
      }
      // Warm-up the iterator
      next();
    }
    int length() { return len(_sskvs); }
    Object key(int idx) { return NonBlockingHashMapi.key(_sskvs, idx); }
    ...
    public void remove() {
      ...
      putIfMatch( NonBlockingHashMapi.this, _sskvs, _prevK, TOMBSTONE, _prevV );
      _prevV = null;
    }
  ...
  public Collection<TypeV> values() {
    return new AbstractCollection<TypeV>() {
      @Override public void    clear   (          ) {        NonBlockingHashMapi.this.clear        ( ); }
      @Override public int     size    (          ) { return NonBlockingHashMapi.this.size         ( ); }
      @Override public boolean contains( Object v ) { return NonBlockingHashMapi.this.containsValue(v); }
  ...
  public Set<TypeK> keySet() {
    return new AbstractSet<TypeK> () {
      @Override public void    clear   (          ) {        NonBlockingHashMapi.this.clear   ( ); }
      @Override public int     size    (          ) { return NonBlockingHashMapi.this.size    ( ); }
      @Override public boolean contains( Object k ) { return NonBlockingHashMapi.this.containsKey(k); }
      @Override public boolean remove  ( Object k ) { return NonBlockingHashMapi.this.remove  (k) != null; }
   ...
   *  iteration.  The {@link NonBlockingHashMapi} does not normally create or
  ...
  public Set<Entry<TypeK,TypeV>> entrySet() {
    return new AbstractSet<Entry<TypeK,TypeV>>() {
      ...
      @Override public boolean remove( final Object o ) {
        ...
        return NonBlockingHashMapi.this.remove(e.getKey(), e.getValue());
      }
